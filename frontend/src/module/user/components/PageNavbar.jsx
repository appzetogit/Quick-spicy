...
export default function PageNavbar({
  ...
  useEffect(() => {
    ...
    const timeoutId = setTimeout(async () => {
      try {
        ...
        if (!isGranted) {
          debugLog("ðŸ“ Geolocation permission not granted; waiting for user action")
          return
        }
        ...
        if (fetchedLocation &&
          ...
          debugLog("âœ… Location fetched successfully:", fetchedLocation)
        } else {
  ...
  const locationDisplay = useMemo(() => {
    ...
    debugLog("ðŸ” PageNavbar - Full Location Object:", {
      location,
    ...
    if (location?.mainTitle && location.mainTitle.trim() !== "" && location.mainTitle !== "Location Found") {
      ...
      debugLog("âœ…âœ…âœ… ZOMATO-STYLE: Using mainTitle for display:", mainLocation);
    }
    ...
    if (!mainLocation && location?.formattedAddress && !isCoordinates(location.formattedAddress) && location.formattedAddress !== "Select location") {
      ...
      if (formattedParts.length >= 4) {
        debugLog("ðŸ” Using formattedAddress (complete address detected):", location.formattedAddress)
        ...
        if (cityIndex > 0) {
          ...
          if (localityParts.length > 0) {
            ...
            debugLog("âœ…âœ…âœ… Extracted locality from complete formattedAddress:", mainLocation)
          }
        } else {
          ...
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(', ')
            debugLog("âœ…âœ…âœ… Using first parts from formattedAddress:", mainLocation)
          }
        }
      }
    }
    ...
    if (!mainLocation && location?.address && !isCoordinates(location.address) && location.address !== "Select location") {
      debugLog("ðŸ” Processing address field (Priority 2):", location.address)
      ...
      if (!cityInAddress && addressParts.length > 0) {
        ...
        debugLog("âœ… Using address field as locality:", mainLocation)
      } else {
        ...
        if (cityIndex > 0) {
          ...
          debugLog("âœ… Extracted locality from address field:", mainLocation)
        } else if (filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          debugLog("âœ… Using first 3 parts from address:", mainLocation)
        }
      }
    }
    ...
    if (!mainLocation && location?.formattedAddress &&
      ...
      location.formattedAddress !== "Select location") {
      debugLog("ðŸ” Processing formattedAddress (Priority 2):", location.formattedAddress)
      const parts = location.formattedAddress.split(',').map(part => part.trim()).filter(part => part.length > 0)
      debugLog("ðŸ“‹ Address parts:", parts)
      ...
      debugLog("ðŸ“‹ Filtered parts (without pincode/country):", filteredParts)
      ...
      if (location?.city) {
        ...
        debugLog(`ðŸ“ City index (exact match with "${location.city}"):`, cityIndex)
      }
      ...
      if (cityIndex === -1) {
        ...
        debugLog("ðŸ“ City index (common cities):", cityIndex)
      }
      ...
      if (cityIndex === -1 && location?.state) {
        ...
        if (stateIndex > 0) {
          ...
          cityIndex = stateIndex - 1
          debugLog("ðŸ“ City index (before state):", cityIndex)
        }
      }
      ...
      if (cityIndex === -1) {
        ...
        if (stateIndex > 0) {
          cityIndex = stateIndex - 1
          debugLog("ðŸ“ City index (before state name):", cityIndex)
        }
      }
      ...
      if (cityIndex > 0) {
        ...
        if (localityParts.length > 0) {
          mainLocation = localityParts.join(', ')
          debugLog("âœ…âœ…âœ… Using exact locality from formattedAddress:", mainLocation)
        } else {
        ...
        if (stateIndex > 0) {
          ...
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(', ')
            debugLog("âœ…âœ…âœ… Using parts before state from formattedAddress:", mainLocation)
          }
        }
        ...
        if (!mainLocation && filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          debugLog("âœ…âœ…âœ… Using first 3 parts from formattedAddress (fallback):", mainLocation)
        } else if (!mainLocation && filteredParts.length >= 2) {
          mainLocation = filteredParts.slice(0, 2).join(', ')
          debugLog("âœ… Using first 2 parts from formattedAddress (fallback):", mainLocation)
          ...
          if (!isCoordinates(firstPart) && firstPart.length > 2) {
            ...
            debugLog("âœ… Using first part from formattedAddress (fallback):", mainLocation)
          }
        }
      }
      ...
      if (!mainLocation) {
        ...
        debugLog("ðŸŽ¯ðŸŽ¯ðŸŽ¯ Final mainLocation extracted:", mainLocation)
      }
    }
    ...
    if (!mainLocation &&
      ...
      debugLog("ðŸ” Processing address field:", location.address)
      ...
      debugLog("ðŸ“‹ Filtered parts:", filteredParts)
      ...
      if (filteredParts.length === 0 ||
        ...
        debugLog("âš ï¸ Address field only contains placeholder, skipping")
        ...
        if (cityIndex > 0) {
          ...
          if (localityParts.length > 0) {
            mainLocation = localityParts.join(', ')
            debugLog("âœ… Using exact locality from address:", mainLocation)
          }
        } else if (filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          debugLog("âœ… Using first 3 parts from address:", mainLocation)
          ...
          mainLocation = filteredParts.slice(0, 2).join(', ')
          debugLog("âœ… Using first 2 parts from address:", mainLocation)
          ...
          if (!isCoordinates(firstPart) &&
            ...
            mainLocation = firstPart
            debugLog("âœ… Using first part from address:", mainLocation)
          }
        }
      }
    }
    ...
    if (!mainLocation) {
      ...
      if (addressToUse &&
        ...
        if (filteredParts.length >= 3) {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          debugLog("âœ… Using first 3 parts as fallback:", mainLocation)
        } else if (filteredParts.length >= 2) {
          mainLocation = filteredParts.slice(0, 2).join(', ')
          debugLog("âœ… Using first 2 parts as fallback:", mainLocation)
        } else if (filteredParts.length >= 1) {
          const firstPart = filteredParts[0]
          if (!isCoordinates(firstPart) && firstPart.length > 2) {
            mainLocation = firstPart
            debugLog("âœ… Using first part as fallback:", mainLocation)
          }
        }
      }
    }
    ...
    if (!mainLocation &&
      ...
      !isCoordinates(location.formattedAddress) &&
      location.formattedAddress !== "Select location") {
      debugLog("ðŸ”„ðŸ”„ðŸ”„ FORCE EXTRACTING from formattedAddress (last resort):", location.formattedAddress)
      ...
      if (filteredParts.length >= 3) {
        ...
        if (thirdPart === "indore" || thirdPart === location?.city?.toLowerCase()) {
          mainLocation = filteredParts.slice(0, 2).join(', ')
          debugLog("âœ…âœ…âœ… FORCE: Using first 2 parts (3rd is city):", mainLocation)
        } else {
          mainLocation = filteredParts.slice(0, 3).join(', ')
          debugLog("âœ…âœ…âœ… FORCE: Using first 3 parts:", mainLocation)
        }
      } else if (filteredParts.length >= 2) {
        mainLocation = filteredParts.slice(0, 2).join(', ')
        debugLog("âœ…âœ…âœ… FORCE: Using first 2 parts:", mainLocation)
      } else if (filteredParts.length >= 1) {
        const firstPart = filteredParts[0]
        if (!isCoordinates(firstPart) && firstPart.length > 2 && firstPart.toLowerCase() !== location?.city?.toLowerCase()) {
          mainLocation = firstPart
          debugLog("âœ…âœ…âœ… FORCE: Using first part:", mainLocation)
        }
      }
    }
    ...
    if (!mainLocation && location?.area && location.area.trim() !== "" && !isCoordinates(location.area)) {
      ...
      if (location?.city && location.city.trim() !== "" && location.city !== "Unknown City" &&
        ...
        debugLog("âœ… Using area + city:", mainLocation)
      } else {
        mainLocation = location.area
        debugLog("âœ… Using area name:", mainLocation)
      }
    }
    ...
    // Skip if city is "Current Location" or "Select location" - these are placeholders
      ...
      debugLog("âš ï¸âš ï¸âš ï¸ FALLBACK: Using city (no locality found):", mainLocation)
    }
    // Final fallback: Show "Select location" instead of coordinates
    else if (!mainLocation) {
      mainLocation = "Select location"
      debugLog("âš ï¸ No valid location found, showing placeholder")
    }
    ...
    if (isCoordinates(mainLocation)) {
      ...
      debugLog("âš ï¸ Replaced coordinates with:", mainLocation)
    }
    ...
    if (mainLocation && (mainLocation.toLowerCase() === location?.city?.toLowerCase() || mainLocation === "Indore")) {
      debugLog("ðŸ”„ðŸ”„ðŸ”„ MainLocation is city, trying to extract locality one more time...")
      ...
      if (location?.area && location.area.trim() !== "" &&
        ...
        mainLocation = `${location.area}, ${location.city}`
        debugLog("âœ…âœ…âœ… Using area from location object:", mainLocation)
        ...
        const parts = location.formattedAddress.split(',').map(p => p.trim()).filter(p => p.length > 0)
        debugLog("ðŸ” Extracting area from formattedAddress parts:", parts)
        ...
        if (cityIndex > 0) {
          ...
          if (areaParts.length > 0) {
            ...
            if (extractedArea && extractedArea.toLowerCase() !== location.city.toLowerCase() &&
              ...
              debugLog("âœ…âœ…âœ… Extracted area from formattedAddress (before city):", mainLocation)
              ...
              if (lastTwoParts.every(p => p.toLowerCase() !== location.city.toLowerCase())) {
                ...
                debugLog("âœ…âœ…âœ… Extracted area (2 parts) from formattedAddress:", mainLocation)
              }
            }
          }
        } else {
          ...
          if (filteredParts.length >= 1) {
            ...
            if (extractedArea && extractedArea.toLowerCase() !== location.city.toLowerCase() &&
              ...
              mainLocation = `${extractedArea}, ${location.city}`
              debugLog("âœ…âœ…âœ… Extracted area (first part) from formattedAddress:", mainLocation)
            }
          }
        }
      }
    ...
    if (mainLocation && mainLocation.toLowerCase() === location?.city?.toLowerCase()) {
      ...
      if (location?.area && location.area.trim() !== "" &&
        ...
        !isCoordinates(location.area)) {
        mainLocation = `${location.area}, ${location.city}`
        debugLog("âœ…âœ…âœ… Added area to city display:", mainLocation)
      }
        ...
        if (parts.length >= 3) {
          ...
          for (let i = parts.length - 3; i >= 0; i--) {
            ...
            if (part && part.toLowerCase() !== location.city.toLowerCase() &&
              ...
              debugLog("âœ…âœ…âœ… Last resort: Extracted area from formattedAddress:", mainLocation)
              break
            }
          }
        }
    ...
    if (hasCity && hasState) {
      ...
      if (location?.formattedAddress) {
        ...
        debugLog("ðŸ“ Extracting city/state from formattedAddress:", {
          formattedAddress: location.formattedAddress,
          parts: parts,
        ...
        if (parts.length >= 4) {
          ...
          debugLog("ðŸ“ Pincode index:", pincodeIndex)
          ...
          if (pincodeIndex > 1 && pincodeIndex !== -1) {
            ...
            debugLog("ðŸ“ Extracted from pincode position:", { cityPart, statePart, pincodeIndex })
            ...
            if (cityPart && statePart &&
              ...
              debugLog("âœ… Using extracted city/state (pincode method):", subLocation)
            }
          }
          ...
          if (!subLocation && parts.length >= 4) {
            ...
            debugLog("ðŸ“ Trying India method:", { lastPart, secondLastPart })
            ...
            if (lastPart === "India" && /^\d{6}$/.test(secondLastPart)) {
              ...
              debugLog("ðŸ“ Extracted from India position:", { cityPart, statePart })
              ...
              if (cityPart && statePart &&
                ...
                statePart.length > 2) {
                subLocation = `${cityPart}, ${statePart}`
                debugLog("âœ… Using extracted city/state (India method):", subLocation)
              }
            }
          }
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            if (pincodeIndex === 6 && parts.length >= 7) {
              ...
              debugLog("ðŸ“ Direct extraction (index method):", { cityPart, statePart, pincodeIndex })
              ...
              if (cityPart && statePart &&
                ...
                statePart.length > 2) {
                subLocation = `${cityPart}, ${statePart}`
                debugLog("âœ… Using extracted city/state (direct index method):", subLocation)
              }
            }
          }
          ...
          if (!subLocation && parts.length >= 6) {
            ...
            debugLog("ðŸ“ Simple fallback (parts[4] and parts[5]):", {
              cityPart,
              statePart,
              partsLength: parts.length,
              allParts: parts
            ...
            if (cityPart && statePart &&
              ...
              statePart.length > 1) {
              subLocation = `${cityPart}, ${statePart}`
              debugLog("âœ… Using extracted city/state (simple fallback):", subLocation)
            } else {
              debugLog("âš ï¸ Validation failed for parts[4] and parts[5]:", { cityPart, statePart })
            }
          }
        }
      }
      ...
      if (!subLocation && location?.address && location.address !== location?.formattedAddress) {
        const parts = location.address.split(',').map(part => part.trim()).filter(part => part.length > 0)
        debugLog("ðŸ“ Trying address field:", { address: location.address, parts })
        ...
        if (parts.length >= 4) {
          ...
          if (pincodeIndex > 1 && pincodeIndex !== -1) {
            ...
            if (cityPart && statePart &&
              ...
              statePart.length > 2) {
              subLocation = `${cityPart}, ${statePart}`
              debugLog("âœ… Using extracted city/state from address field:", subLocation)
            }
          }
        }
      }
      ...
      if (!subLocation) {
        subLocation = ""
        debugLog("âš ï¸ Could not extract city/state from address")
      }
    }

    // Debug log
    debugLog("ðŸ“ PageNavbar Location Display:", {
      location: location,
      city: location?.city,
      state: location?.state,
      hasCity,
      ...
      if (location?.formattedAddress) {
        ...
        if (parts.length >= 6) {
          ...
          if (cityPart && statePart &&
            ...
            statePart.length > 1) {
            subLocation = `${cityPart}, ${statePart}`
            debugLog("âœ…âœ…âœ… FORCED extraction - New subLocation:", subLocation)
          }
        }
      }
    }
